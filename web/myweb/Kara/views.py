...
class waiting_view(View):
    ...
    def post(self,request,*args, **kwargs):
        info={}
        global path_song,lyric,song_name,author_name,function_lyric
        if(song_search!=''):
            lyric,number,path_song,song_name,author_name=take_lyric_song(song_search)
            function_lyric=None
        else:
            path_song   =os.path.join(os.getcwd(),'media','wav',str(file.objects.all().order_by('-id')[0].vocal_file))
            lyric       =os.path.join(os.getcwd(),'media','txt',str(file.objects.all().order_by('-id')[0].lyric_file))
            song_name   ="NULL"
            author_name ="NULL"
            function_lyric=get_lyric
            

        word,treils_length,vocal=Lyrics_to_alignment(path_song,lyric,get_lyric_function=function_lyric,get_vocal_function=get_vocal).run()
