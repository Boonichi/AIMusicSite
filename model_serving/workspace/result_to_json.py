...
def convert_to_json_form(word_segments,original_lyrics,number_sentence_word_per_sentence,trellis_len,len_signal):
    result=[]
    index=0
    ...
    for w in original_lyrics:
        ...
        if(not check_special_sign(w)): 
            result[index_sentence]['w'][count_word]['s']=converto_time(word_segments[index].start,trellis_len,len_signal,16000)
            result[index_sentence]['w'][count_word]['d']=w
            result[index_sentence]['w'][count_word]['e']=converto_time(word_segments[index].end,trellis_len,len_signal,16000)
            index +=1
        else: #some special situation
            result[index_sentence]['w'][count_word]['s']=converto_time(word_segments[index].start,trellis_len,len_signal,16000)
            result[index_sentence]['w'][count_word]['d']=w
            result[index_sentence]['w'][count_word]['e']=converto_time(word_segments[index+1].end,trellis_len,len_signal,16000)
            index +=2
        count_word+=1
